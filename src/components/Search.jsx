        ...
        <ul className="search-movieList">
          {
            movies.map((movie) => (
              <li key={movie.id}>
                <Link to={`/movies/${movie.id}`} onClick={()=>{showDetail(movie.id)}}>
                  <div className="search-list">
                    <div className="img">
                        <img src={`${imgPath}${movie.poster_path}`} style={{height: "300px"}} alt={movie.title} />
                    </div>
                    <div className="info">
                        <p className="title" style={{color: "#fff"}}>{movie.title}</p>
                        <p className="release" style={{color: "#aaa", fontSize: "14px"}}>{movie.release_date}</p>
                    </div>
                  </div>
                </Link>
              </li> 
